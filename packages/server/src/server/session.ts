...
// Cloud repo-provisioning (DynamoDB + Secrets-Manager GitHub token) imports are
// preserved; error-utils/agent-state-bucket now live in @getpaseo/protocol
// (HEAD's ../shared/* paths no longer exist). getParentAgentIdFromLabels is
// upstream's and is used live below.
...
import {
  ...
  // Cloud add-project / create-project path (createProjectInWorkspace,
  // persistCheckoutRecordForProject) consumes these.
...
import {
  ...
  // DEFAULT_CONTAINER_WORKSPACE_ID (cloud container model) + upstream's
  // resolve*DisplayName/Name helpers are all used below.
...
// Scoped-env (D-3.5c) store/validator preserved. buildVoiceModeSystemPrompt /
// stripVoiceModeSystemPrompt were dropped — voice prompt shaping moved into
// session/voice/; only wrapSpokenInput is still used in the coordinator.
...
// MERGE FLAG (chat-service naming): the cross-file contract mandates the class
// name `ChatService` (constructed `new ChatService({ store })` in bootstrap),
// and the working-tree chat-service.ts exports `ChatService`. Upstream renamed
// it `FileBackedChatService`. session.ts uses `ChatService` to match the
// contract + the constructed value. The decomposed sibling
// session/chat/chat-schedule-loop-session.ts still imports/types
// `FileBackedChatService`; that sibling (outside the 3 merge files) must be
// reconciled to `ChatService`, OR chat-service.ts must export a
// `FileBackedChatService` alias. The dead inline chat handlers (which used
// ChatServiceError / parseMentionAgentIds / chat-mentions) were dropped — chat
// dispatch routes to chatScheduleLoopSession.
...
// DeleteFencedAgentStorage intersects AgentStore (the interface this fork's
// stores implement, incl. DynamoAgentStore) so the cast below stays valid for
// the AgentStore-typed `agentStorage` parameter. (Dropped HEAD's unused
// clientSupportsFlexibleEditorIds helper.)
...
// when the user edits it (daemon OQ-3). Used by the scoped-env-var handlers.
// (Dropped HEAD's unused VoiceTranscriptionResultPayload — voice transcription
// moved into session/voice/.)
...
export interface SessionOptions {
  ...
  // Upstream's injectable filesystem seam (used by workspace/project create).
  filesystem?: SessionFileSystem;
  // chatService typed ChatService per the cross-file contract (see import note).
...
export class Session {
  ...
  // Cloud workspace-JWT scoping of the loopback MCP transport (cloud-only).
  // downloadTokenStore is forwarded directly to subsystems, so no field is kept.
  ...
  constructor(options: SessionOptions) {
    ...
    this.terminalController = new TerminalSessionController({
      ...
      // Cloud D-3.5c scoped-env into terminals, merged with upstream's new
      // terminal controller callbacks.
  ...
  private async initializeAgentMcp(): Promise<void> {
    try {
      ...
      // MERGE FLAG (per-session MCP Bearer): cloud and upstream use DIFFERENT
      // token sources for the loopback /mcp/agents transport.
      //   - Cloud: the workspace JWT captured at WS upgrade — the cloud daemon
      //     gates /mcp/agents behind the require-workspace middleware.
      //   - On-host: upstream's agentManager.getMcpAuthToken() — gated behind
      //     the require-bearer middleware with the daemon MCP auth token.
      // Resolved as a hybrid: prefer the workspace JWT (cloud), else fall back
      // to the on-host MCP auth token. VERIFY this precedence is correct (cloud
      // require-workspace must accept the workspace JWT; on-host require-bearer
      // must accept getMcpAuthToken()).
      const authToken = this.workspaceToken ?? this.agentManager.getMcpAuthToken();
  ...
  private dispatchAgentConfigMessage(msg: SessionInboundMessage): Promise<void> | undefined {
    switch (msg.type) {
      ...
      case "write_project_config_request":
        // Project-config handlers moved to the project-config subsystem upstream.
        return this.projectConfigSession.handleWriteProjectConfigRequest(msg);
      // D-3.5c scoped-env-var RPCs — no upstream subsystem; handled inline.
  ...
  // D-3.5c — scoped env-var RPCs (cloud-fork; no upstream subsystem). `secret`
  // values are write-only: `list` returns a masked placeholder, never the
  // stored value (daemon OQ-3). Key validation (charset, length, reserved-key
  // reject) is shared with the resolver's denylist via `validateEnvVarKeyValue`.
  ...
  // NOTE (merge): HEAD's inline handleReadProjectConfigRequest /
  // handleWriteProjectConfigRequest / emitProjectConfig*Failure were dropped —
  // project-config handling moved to session/project-config/ upstream and the
  // dispatch now routes to this.projectConfigSession.
  // eslint-disable-next-line complexity
  ...
  // Upstream (project.add.request) — add a project by directory path. Distinct
  // from the cloud container-scoped add_project flow above.
