...
// MERGE(upstream-0.1.99): upstream converged on its own workspace/project model
// (WorkspaceCreateRequestSchema below + projectAdd/projectRemove capabilities).
// Both schema sets are kept to compile; Step 3b converges callers onto upstream's
// types, grafting the cloud-only `github_repo` source onto upstream's source union.
