...
>["payload"];
// Cloud (D-3.5a) — the workspace-scoped Project add and the name-only Workspace
// container create. Distinct from upstream's directory-oriented addProject(cwd)
// / createWorkspace({source}); these carry the cloud GitHub-repo source and the
// ambient-container semantics (see add_project_request / create_workspace_request).
type AddProjectCloudPayload = Extract<
  SessionOutboundMessage,
  { type: "add_project_response" }
>["payload"];
type CreateWorkspaceCloudPayload = Extract<
  SessionOutboundMessage,
...
export class DaemonClient {
  ...
  // Cloud (D-3.5a, app T-1) — add a Project (local dir or GitHub repo) to a
  // Workspace container. The cloud GitHub-repo source can't be expressed via
  // upstream's directory-only addProject(cwd); this sends `add_project_request`
  // with `{workspaceId, source}`. The GitHub path may clone, so allow a longer
  // timeout. Consumed by github-repo-picker.tsx.
  async addProjectFromSource(
    input: { workspaceId: string; source: AddProjectSource },
    requestId?: string,
  ): Promise<AddProjectCloudPayload> {
    return this.sendCorrelatedSessionRequest({
      requestId,
      message: {
  ...
  // Cloud (D-3.5a, app T-1) — create a Workspace container by name only (no repo,
  // no directory). On-host mints a `ws_<uuid>`; cloud returns the ambient
  // container (idempotent). Distinct from upstream's createWorkspace({source}).
  // Consumed by project-picker-modal.tsx.
  async createWorkspaceContainer(
    displayName: string,
    requestId?: string,
  ): Promise<CreateWorkspaceCloudPayload> {
    return this.sendCorrelatedSessionRequest({
      requestId,
      message: {
