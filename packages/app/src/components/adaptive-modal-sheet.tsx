...
export const AdaptiveTextInput = forwardRef<TextInput, AdaptiveTextInputProps>(
  function AdaptiveTextInputInner(props, ref) {
    ...
    // BottomSheetTextInput calls useBottomSheetInternal(), which throws
    // ("'useBottomSheetInternal' cannot be used out of the BottomSheet!") when
    // there is no <BottomSheet> ancestor. Several cloud screens render this input
    // outside a sheet (e.g. /orchestra/setup, provider-credential settings), so we
    // detect the context first — the `true` overload returns null instead of
    // throwing — before reaching for BottomSheetTextInput.
