...
export interface ResolveIndexStartupRouteInput extends ResolveStartupRouteBaseInput {
  ...
  hasGivenUpWaitingForHost: boolean;
  // Orchestra cold-load barrier (re-homed from the old app/ bootstrap during the
  // upstream-0.1.99 merge). Whether the online host may be auto-entered on a cold
  // load. The CALLER computes this (it holds the full host profiles + platform)
  // via isAutoRoutableOnlineHost: a cloud host on web is NOT auto-routable, so a
  // cold SPA reload must not bounce into the existing workspace and skip the
  // create-a-new-workspace wizard. Optional/undefined → auto-routable (preserves
  // upstream behavior and keeps upstream's resolver tests unchanged).
  canAutoRouteOnlineHost?: boolean;
}
...
function resolveReadyIndexStartupRoute(input: ResolveIndexStartupRouteInput): StartupRouteDecision {
  ...
  // Orchestra cold-load barrier: on web a cloud workspace host must not be
  // auto-entered on cold load — neither via workspace-selection restore nor via
  // the online-host redirect below — or the reload bounces into the existing
  // workspace and the welcome → create-a-new-workspace wizard is unreachable.
  // The caller sets canAutoRouteOnlineHost=false for a cloud host on web.
  if (input.anyOnlineHostServerId && input.canAutoRouteOnlineHost === false) {
